...
proof-
  have \<open>bounded_clinear (fun_to_ell2 n f)\<close>
  proof-
    have \<open>clinear (fun_to_ell2 n f)\<close>
      by (simp add: assms clinear_ell2_map_left)      
    moreover have \<open>j \<ge> n \<Longrightarrow> (fun_to_ell2 n f) (ket j) = 0\<close>
      for j
    proof-
      assume \<open>j \<ge> n\<close>
      have  \<open>f (Abs_vec
        (n, \<lambda>i. if i < n then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                else undef_vec (i - n))) = f (0\<^sub>v n)\<close>
      proof-
        have \<open>i < n \<Longrightarrow> vec_index ( Abs_vec
        (n, \<lambda>i. if i < n then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                else undef_vec (i - n)) ) i = vec_index (0\<^sub>v n) i\<close>
          for i
        proof-
          assume \<open>i < n\<close>
          have \<open>vec_index ( Abs_vec
        (n, \<lambda>i. if i < n then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                else undef_vec (i - n)) ) i = 0\<close>
          proof-
            have \<open>snd (n, \<lambda>i. if i < n then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                    else undef_vec (i - n)) i = 0\<close>
            proof-
              have \<open>Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i = 0\<close>
              proof-
                have \<open>(\<lambda>y. if j = y then 1 else 0) i = 0\<close>
                  using \<open>i < n\<close> \<open>n \<le> j\<close> by auto
                moreover have \<open>Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0))
                    = (\<lambda>y. if j = y then 1 else 0)\<close>
                proof-
                  have \<open>(\<lambda>y. if j = y then 1 else 0) \<in> {x. has_ell2_norm x}\<close>
                    unfolding has_ell2_norm_def bdd_above_def apply simp
                    apply (rule exI[of _ 1], rule allI, rule impI)
                    by (rule ell2_1)
                  thus ?thesis using Abs_ell2_inverse by blast
                qed
                ultimately show ?thesis by simp
              qed
              hence \<open>(if i < n then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                    else undef_vec (i - n)) = 0\<close>
                using \<open>i < n\<close> by simp
              thus ?thesis by simp
            qed
            moreover have \<open>Rep_vec (Abs_vec (n, \<lambda>i. if i < n 
                    then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                    else undef_vec (i - n))) = (n, \<lambda>i. if i < n 
                    then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                    else undef_vec (i - n))\<close>
            proof-
              have \<open>(n, \<lambda>i. if i < n 
                    then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                    else undef_vec (i - n)) \<in> {(n, mk_vec n f) |n f. True}\<close>
                unfolding mk_vec_def
                by auto
              thus ?thesis using Abs_vec_inverse by blast
            qed
            ultimately show ?thesis apply transfer
              by (simp add: vec_index.rep_eq) 
          qed
          thus ?thesis
            by (simp add: \<open>i < n\<close>) 
        qed
        moreover have \<open>dim_vec (Abs_vec
        (n, \<lambda>i. if i < n then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                else undef_vec (i - n))) = n\<close>
        proof-
          have \<open>fst (n, \<lambda>i. if i < n then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
                    else undef_vec (i - n)) = n\<close>
            by simp
          moreover have \<open>Rep_vec (Abs_vec (n, \<lambda>i. if i < n 
                    then Rep_ell2 (Abs_ell2 (\<lambda>y. if j = y then 1 else 0)) i
          ...
          proof-
            ...
            thus ?thesis using Abs_vec_inverse by blast
          qed
          ultimately show ?thesis unfolding dim_vec_def by auto
        qed
        moreover have \<open>dim_vec (0\<^sub>v n) = n\<close>
          by simp          
      ...
      ultimately have \<open>(fun_to_ell2 n f) (ket j) = 0\<close>
        unfolding fun_to_ell2_def vec_def mk_vec_def id_def ket_def
        by auto
      thus ?thesis by blast
    qed
    ultimately show ?thesis using ell2_bounded by blast
  qed
  thus ?thesis unfolding finite_rank_operator_def
    using assms finite_complex_rank_def finite_complex_rank_ell2_map_left_vec_exact 
    by blast 
qed
