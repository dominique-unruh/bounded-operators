...
subsection\<open>Free Vector Spaces\<close>
...
proof
  ...
  show "(a + b) *\<^sub>R x = a *\<^sub>R x + b *\<^sub>R x"
    for a :: real
      ...
      and x :: "'a free"
    apply transfer
    by (metis of_real_add scaleC_add_left) 
...
lemma support_superset:
  ...
  shows \<open>(\<Sum>z\<in>S. (x \<down> z) *\<^sub>C f z) = (\<Sum>z\<in>{u |u. x \<down> u \<noteq> 0}. (x \<down> z) *\<^sub>C f z)\<close>
proof-
...
functor free_map
proof
  show "(free_map f \<circ> free_map g) x = free_map (f \<circ> g) x"
      ...
      proof-
        ...
        proof-
          ...
          ultimately have \<open>f (\<Sum>a\<in>S. a) = (\<Sum>a\<in>S. f a)\<close>
            unfolding S_def
            using Finite_Cartesian_Product.sum_cong_aux additive.sum additive_def
...
subsection\<open>Algebraic Tensor Product\<close>
...
proof-
  ...
  show \<open>c *\<^sub>R p1 - c *\<^sub>R p2 \<in> atensor_kernel\<close>
    by (metis \<open>c *\<^sub>R (p1 - p2) \<in> atensor_kernel\<close> real_vector.scale_right_diff_distrib)    
qed
...
instance
  ...
  show "a + b + c = a + (b + c)"
    ...
    apply transfer
    by (metis ab_semigroup_add_class.add_ac(1) atensor.abs_eq_iff) 
  show "a + b = b + a"
    for a :: "'a \<otimes>\<^sub>a 'b"
      and b :: "'a \<otimes>\<^sub>a 'b"
    apply transfer
    by (metis add.commute atensor.abs_eq_iff) 
  show "(0::'a \<otimes>\<^sub>a 'b) + a = a"
    for a :: "'a \<otimes>\<^sub>a 'b"
    apply transfer
    using atensor.abs_eq_iff by auto 
  show "- a + a = 0"
    for a :: "'a \<otimes>\<^sub>a 'b"
    apply transfer
    using atensor.abs_eq_iff by auto 
  ...
  show "a *\<^sub>R (x + y) = a *\<^sub>R x + a *\<^sub>R y"
    ...
    apply transfer
    by (metis atensor.abs_eq_iff scaleR_add_right) 
  show "(a + b) *\<^sub>R x = a *\<^sub>R x + b *\<^sub>R x"
    for a :: real
      and b :: real
      and x :: "'a \<otimes>\<^sub>a 'b"
    apply transfer
    by (metis atensor.abs_eq_iff scaleR_left.add) 
  show "a *\<^sub>R b *\<^sub>R x = (a * b) *\<^sub>R x"
    for a :: real
      and b :: real
      and x :: "'a \<otimes>\<^sub>a 'b"
  ...
  show "1 *\<^sub>R x = x"
    ...
    apply transfer
    by (simp add: atensor_equivp equivp_reflp) 
  show "a *\<^sub>C (x + y) = a *\<^sub>C x + a *\<^sub>C y"
    for a :: complex
      and x :: "'a \<otimes>\<^sub>a 'b"
      and y :: "'a \<otimes>\<^sub>a 'b"
    apply transfer
    by (metis atensor.abs_eq_iff scaleC_add_right) 
  show "(a + b) *\<^sub>C x = a *\<^sub>C x + b *\<^sub>C x"
    for a :: complex
      and b :: complex
      and x :: "'a \<otimes>\<^sub>a 'b"
    apply transfer
    by (metis atensor.abs_eq_iff scaleC_add_left) 
  show "a *\<^sub>C b *\<^sub>C x = (a * b) *\<^sub>C x"
    for a :: complex
      and b :: complex
      and x :: "'a \<otimes>\<^sub>a 'b"
  ...
  show "((*\<^sub>R) r::'a \<otimes>\<^sub>a 'b \<Rightarrow> _) = (*\<^sub>C) (complex_of_real r)"
    ...
    apply auto
  proof-
    have \<open>(((*\<^sub>R) r)::('a \<otimes>\<^sub>a 'b \<Rightarrow> _)) = ((*\<^sub>C) (complex_of_real r))\<close>
    proof
      show "r *\<^sub>R x = complex_of_real r *\<^sub>C x"
        for x :: "'a \<otimes>\<^sub>a 'b"
        apply transfer
        by (metis (full_types) atensor.abs_eq_iff scaleR_scaleC)  
    qed      
    thus \<open>map_fun rep_atensor abs_atensor ((*\<^sub>R) r) =
...
proof-
  ...
  moreover have \<open>abs_atensor X = (\<Sum>z\<in>{u | u.  X \<down> u \<noteq> 0}.  (X \<down> z) *\<^sub>C ( (fst z) \<otimes>\<^sub>a (snd z) ) )\<close>
    by (metis (mono_tags, lifting) case_prod_unfold free_explicit' sum.cong)     
...
subsection\<open>Hilbert Tensor Product\<close>
