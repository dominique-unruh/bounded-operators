...
proof -
  {
    ...
    also have "\<dots> \<le>  norm i * norm (f i)"
      by (rule mult_right_mono)
        (auto simp: ac_simps Inner_Product.Cauchy_Schwarz_ineq2 intro!: onorm_le)
...
(* Sharing the same attribute [bounded_linear_intros] with Bounded_Linear_Function *)
...
(* TODO: same for bounded_antilinear *)
...
(* TODO: same for bounded_sesquilinear *)
...
lemma bounded_linear_cblinfun_apply[bounded_linear_intros]:
...
lemma cbounded_linear_cBlinfun_apply: "bounded_clinear f \<Longrightarrow> cblinfun_apply (cBlinfun f) = f"
