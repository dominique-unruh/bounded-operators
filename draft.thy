...
section \<open>Real pseudo-normed space\<close>
...
class real_pseudo_normed_vector = real_vector + pseudo_norm + pseudo_dist +
  assumes pseudo_norm_eq_zero [simp]: "x = 0 \<longrightarrow> pseudo_norm x = 0"
    ...
    and pseudo_norm_pseudo_dist: \<open>pseudo_dist x y = pseudo_norm (x - y)\<close>
begin 

lemma peudo_norm_ge_zero [simp]: "0 \<le> pseudo_norm x"
proof -

  have "0 = pseudo_norm (x + -1 *\<^sub>R x)"
  proof-
    have \<open>x + -1 *\<^sub>R x = (0::'a)\<close>
      by (metis (no_types, hide_lams) group_add_class.add_minus_cancel local.add_cancel_right_right local.scaleR_add_left local.scaleR_one)
    moreover have \<open>pseudo_norm (0::'a) = 0\<close>
      by simp
    ultimately show ?thesis by simp
  qed
  also have "\<dots> \<le> pseudo_norm x + pseudo_norm (-1 *\<^sub>R x)" 
    by (rule pseudo_norm_triangle_ineq)
  finally show ?thesis by simp
qed

lemma triangular_ineq: \<open>pseudo_dist x y \<le> pseudo_dist x z + pseudo_dist z y\<close>
  using pseudo_norm_pseudo_dist pseudo_norm_triangle_ineq
  by (metis (mono_tags, lifting) local.add.left_commute local.diff_add_cancel local.uminus_add_conv_diff)
...
instantiation pseudo_completion :: (real_normed_vector) real_pseudo_normed_vector
begin
...
lift_definition  plus_pseudo_completion ::
  \<open>'a pseudo_completion \<Rightarrow> 'a pseudo_completion \<Rightarrow> 'a pseudo_completion\<close>
  is \<open>\<lambda> x y. (\<lambda> n. x n + y n)\<close>
  by (rule CauchySEQ_add)

lift_definition pseudo_norm_pseudo_completion :: \<open>'a pseudo_completion \<Rightarrow> real\<close>
...
lift_definition pseudo_dist_pseudo_completion :: \<open>'a pseudo_completion \<Rightarrow> 'a pseudo_completion \<Rightarrow> real\<close>
...
proof
  ...
  show "1 *\<^sub>R (x::'a pseudo_completion) = x"
    ...
    apply transfer by auto
  show "(x::'a pseudo_completion) = 0 \<longrightarrow> pseudo_norm x = 0"
    for x :: "'a pseudo_completion"
    apply transfer
  proof
    ...
    proof-
      have \<open>(\<lambda>n. norm (x n)) = (\<lambda>n. 0)\<close>
        by (simp add: that(2))
      moreover have \<open>lim (\<lambda>n. 0::real) = 0\<close>
        by simp
      ultimately show ?thesis by simp
    qed
  qed
  show "pseudo_norm ((x::'a pseudo_completion) + y) \<le> pseudo_norm x + pseudo_norm y"
    for x :: "'a pseudo_completion"
      and y :: "'a pseudo_completion"
  proof transfer
    ...
    have \<open>Cauchy (\<lambda> n. (x n + y n))\<close>
      using \<open>Cauchy x\<close> \<open>Cauchy y\<close>  CauchySEQ_add
  ...
  show "pseudo_norm (a *\<^sub>R (x::'a pseudo_completion)) = \<bar>a\<bar> * pseudo_norm x"
    for a :: real
      and x :: "'a pseudo_completion"
    apply transfer
  proof-
  ...
  show \<open>pseudo_dist (x::'a pseudo_completion) y = pseudo_norm (x - y)\<close>
    for x :: "'a pseudo_completion"
      and y :: "'a pseudo_completion"              
    apply transfer by auto
qed
