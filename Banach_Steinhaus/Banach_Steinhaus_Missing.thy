...
proof-
  ...
  hence \<open>\<exists> a \<in> *s* A. hnorm (star_of r - a)
           < hypreal_of_real ( (\<lambda>n. inverse (real (Suc n))) n )\<close>
    for n::nat
    by (meson InfinitesimalD2 \<open>\<exists>a\<in>*s* A. star_of r \<approx> a\<close> bex_Infinitesimal_iff inverse_positive_iff_positive of_nat_0_less_iff zero_less_Suc)
...
lemma Cauchy_cinner_Cauchy:
  fixes x y :: \<open>nat \<Rightarrow> 'a::real_inner\<close>
  assumes \<open>Cauchy x\<close> and \<open>Cauchy y\<close>
  shows \<open>Cauchy (\<lambda> n. inner (x n) (y n) )\<close>
proof-
  ...
  have \<open>e > 0 \<Longrightarrow> \<exists> N. \<forall> n \<ge> N. \<forall> m \<ge> N. norm ( (\<lambda> i. inner (x i) (y i) ) n -  (\<lambda> i. inner (x i) (y i)) m ) < e\<close>
    for e
  proof-
    assume \<open>e > 0\<close>
    ...
    have \<open>N \<ge> N2\<close>
      ...
      by auto
    have \<open>n \<ge> N \<Longrightarrow> m \<ge> N \<Longrightarrow> norm ( inner (x n) (y n)  - inner (x m) (y m)  ) < e\<close>
      for n m
    proof-
      assume \<open>n \<ge> N\<close> and \<open>m \<ge> N\<close>
      have \<open>inner (x n) (y n) - inner (x m) (y m)  = inner (x n) (y n)  - inner  (x m) (y n)  + inner (x m) (y n) - inner (x m) (y m)\<close>
        by simp
      hence \<open>norm (inner (x n) (y n)  - inner (x m) (y m) ) \<le> norm ( inner (x n) (y n)  - inner (x m) (y n) )
           + norm (inner (x m) (y n)  - inner (x m) (y m) )\<close>
        by (smt norm_triangle_ineq)        
      moreover have \<open>norm ( inner (x n) (y n)  - inner  (x m) (y n) ) < e/2\<close>
      proof-
        have \<open>inner (x n) (y n)  - inner (x m) (y n)  = inner (x n - x m) (y n)\<close>
          by (simp add: inner_diff_left)          
        hence \<open>norm (inner (x n) (y n)  - inner (x m) (y n) ) = norm ( inner (x n - x m) (y n) )\<close>
          by simp
        moreover have \<open>norm (inner (x n - x m) (y n))  \<le> norm (x n - x m) * norm (y n)\<close>
        proof-
          have \<open>inner (x n - x m) (y n)  \<le> norm (x n - x m) * norm (y n)\<close>
            using Inner_Product.real_inner_class.norm_cauchy_schwarz[where x = "x n - x m" and y = "y n"]
            by auto
          moreover have \<open>- inner (x n - x m) (y n)  \<le> norm (x n - x m) * norm (y n)\<close>
            by (metis inner_minus_left minus_diff_eq norm_cauchy_schwarz norm_minus_commute)
          ultimately show ?thesis by auto
        qed
        ...
        ultimately have \<open>norm (inner (x n) (y n)  - inner (x m) (y n) ) < (e/(2*M)) * M\<close>
          by (smt mult_cancel_right norm_ge_zero real_mult_le_cancel_iff2 real_mult_less_iff1)                    
        moreover have \<open> (e/(2*M)) * M = e/2\<close>
          using \<open>M > 0\<close> by simp
        ultimately have  \<open>norm (inner (x n) (y n)  - inner (x m) (y n)) < e/2\<close>
          by simp
        thus ?thesis by blast
      qed
      moreover have \<open>norm (inner (x m) (y n)  - inner (x m) (y m)) < e/2\<close>
      proof-
        have \<open>inner (x m) (y n)  - inner (x m) (y m)  = inner (x m) (y n - y m)\<close>
          by (simp add: inner_diff_right)          
        hence \<open>norm (inner (x m) (y n)  - inner (x m) (y m) ) = norm ( inner (x m) (y n - y m) )\<close>
          by simp
        moreover have \<open>norm (inner (x m) (y n - y m) ) \<le> norm (x m) * norm (y n - y m)\<close>
        proof-
          have \<open>inner (x m) (y n - y m) \<le> norm (x m) * norm (y n - y m)\<close>
            by (simp add: norm_cauchy_schwarz)            
          moreover have \<open>-inner (x m) (y n - y m) \<le> norm (x m) * norm (y n - y m)\<close>
            by (metis inner_minus_right minus_diff_eq norm_cauchy_schwarz norm_minus_commute)            
          ultimately show ?thesis by auto
        qed
        ...
        ultimately have \<open>norm (inner (x m) (y n)  - inner (x m) (y m) ) < M * (e/(2*M))\<close>
          by (smt linordered_semiring_strict_class.mult_strict_mono norm_ge_zero)
        moreover have \<open>M * (e/(2*M)) = e/2\<close>
          using \<open>M > 0\<close> by simp
        ultimately have  \<open>norm (inner (x m) (y n) - inner (x m) (y m)) < e/2\<close>
          by simp
        thus ?thesis by blast
      qed
      ultimately show \<open>norm ( inner (x n) (y n)  - inner (x m) (y m)) < e\<close> by simp
    qed
    thus ?thesis by blast
  qed
  thus ?thesis
...
lemma Cauchy_cinner_convergent:
  fixes x y :: \<open>nat \<Rightarrow> 'a::real_inner\<close>
  assumes \<open>Cauchy x\<close> and \<open>Cauchy y\<close>
  shows \<open>convergent (\<lambda> n. inner (x n) (y n) )\<close>
proof-
  have \<open>Cauchy (\<lambda> n. inner (x n) (y n) )\<close>
    using \<open>Cauchy x\<close> \<open>Cauchy y\<close> Cauchy_cinner_Cauchy
    by blast
  hence \<open>Cauchy (\<lambda> n. norm (inner (x n) (y n) ))\<close>
    using Cauchy_convergent_norm by blast    
  hence \<open>convergent (\<lambda> n. norm (inner (x n) (y n)))\<close>
    using Cauchy_convergent_iff by auto
  thus ?thesis
    using Cauchy_convergent_iff \<open>Cauchy (\<lambda>n. inner (x n) (y n))\<close> by auto
qed
...
lemma bounded_clinear_Cauchy:
  assumes \<open>Cauchy x\<close> and \<open>bounded_linear f\<close>
...
proof-
  ...
  proof-
    ...
    have \<open>\<exists> M. \<forall> t. norm (f t) \<le> norm t * M \<and> M > 0\<close>
      using assms(2) bounded_linear.bounded_linear bounded_linear.pos_bounded
      by blast
