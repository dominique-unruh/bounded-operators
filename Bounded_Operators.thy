...
proof transfer
  show "F (b1 + b2) = F b1 + F b2"
    ...
    for F :: "'b \<Rightarrow> 'a"
      and b1 b2 :: 'b
    using that unfolding bounded_clinear_def clinear_def Vector_Spaces.linear_def
...
(* TODO: Show that this is unitary? Or at least an iso *)
...
proof intro_classes
  ...
  proof -
    ...
    have "A x = one_dim_iso (A 1) *\<^sub>C one_dim_iso x" for x
      by (smt (z3) cblinfun_apply_scaleC complex_vector.scale_left_commute one_dim_iso_idem one_dim_scaleC_1)
...
(* TODO: prove. *)
(* Note: this is more general than positive_op_def since B does not have to have the same type as A *)
(* lemma positive_opI: \<open>A = B* o\<^sub>C\<^sub>L B \<Longrightarrow> positive_op A\<close> *)
...
definition less_eq_cblinfun_def_heterogenous: \<open>less_eq_cblinfun A B = 
  (if kind_of_same_type TYPE('a) TYPE('b) then
    \<exists>C :: 'b\<Rightarrow>\<^sub>C\<^sub>L'b. (B-A) o\<^sub>C\<^sub>L (heterogenous_cblinfun_id :: 'b\<Rightarrow>\<^sub>C\<^sub>L'a) = C* o\<^sub>C\<^sub>L C else (A=B))\<close>
definition \<open>less_cblinfun (A :: 'a \<Rightarrow>\<^sub>C\<^sub>L 'b) B \<longleftrightarrow> A \<le> B \<and> \<not> B \<le> A\<close>
instance..
end

lemma less_eq_cblinfun_def: \<open>A \<le> B = 
    (\<exists>C :: 'a::chilbert_space \<Rightarrow>\<^sub>C\<^sub>L 'a. B-A = C* o\<^sub>C\<^sub>L C)\<close>
  unfolding less_eq_cblinfun_def_heterogenous
  by auto

instantiation cblinfun :: (chilbert_space, chilbert_space) order begin
instance
proof intro_classes
  fix x y z :: \<open>'a \<Rightarrow>\<^sub>C\<^sub>L 'b\<close>
  define pos where \<open>pos X \<longleftrightarrow> (\<exists>C :: 'b \<Rightarrow>\<^sub>C\<^sub>L 'b. X = C* o\<^sub>C\<^sub>L C)\<close> for X
  ...
  have [simp]: \<open>pos 0\<close>
    unfolding pos_def apply (rule exI[of _ 0]) by auto

  have pos_nondeg: \<open>X = 0\<close> if \<open>pos X\<close> and \<open>pos (-X)\<close> for X
  proof -
    from \<open>pos X\<close> obtain Y :: \<open>'b \<Rightarrow>\<^sub>C\<^sub>L 'b\<close> where XY: \<open>X = Y* o\<^sub>C\<^sub>L Y\<close>
      using pos_def by blast
    from \<open>pos (-X)\<close> obtain Z :: \<open>'b \<Rightarrow>\<^sub>C\<^sub>L 'b\<close> where XZ: \<open>-X = Z* o\<^sub>C\<^sub>L Z\<close>
      using pos_def by blast
    { fix \<psi> 
      have \<open>of_real (norm (Y *\<^sub>V \<psi>)) ^ 2 = cinner \<psi> (X *\<^sub>V \<psi>)\<close>
        by (auto simp: XY cinner_adj_right power2_norm_eq_cinner)
      moreover have \<open>cinner \<psi> (-X *\<^sub>V \<psi>) \<ge> 0\<close>
        using cinner_ge_zero by (auto simp: XZ cinner_adj_right)
      ultimately have \<open>norm (Y *\<^sub>V \<psi>) = 0\<close>
        by (metis (no_types, hide_lams) add.inverse_inverse antisym cinner_ge_zero cinner_minus_right cnorm_eq neg_le_0_iff_le norm_zero power2_norm_eq_cinner uminus_cblinfun.rep_eq)
      then have \<open>Y *\<^sub>V \<psi> = 0\<close>
        by auto
    }
    then have \<open>Y = 0\<close>
      by (simp add: cblinfun_eqI)
    then show \<open>X = 0\<close>
      by (auto simp: XY)
  qed
...
lemma positive_id_cblinfun[simp]: "id_cblinfun \<ge> 0"
  unfolding less_eq_cblinfun_def apply (rule exI[of _ id_cblinfun]) by simp

lemma cblinfun_scaleC_pos_mono: 
  fixes X :: \<open>'a::chilbert_space \<Rightarrow>\<^sub>C\<^sub>L 'a\<close>
  assumes \<open>X \<ge> 0\<close>
  assumes \<open>c \<le> d\<close>
  shows \<open>c *\<^sub>C X \<le> d *\<^sub>C X\<close>
proof -
  from \<open>X \<ge> 0\<close> obtain Y :: \<open>'a \<Rightarrow>\<^sub>C\<^sub>L 'a\<close> where [simp]: "X = Y* o\<^sub>C\<^sub>L Y"
    by (metis diff_zero less_eq_cblinfun_def)
  from \<open>c \<le> d\<close> obtain \<delta> where [simp]: \<open>d - c = of_real \<delta>\<close> \<open>\<delta> \<ge> 0\<close>
    by (metis complex_of_real_cmod diff_ge_0_iff_ge norm_ge_zero)
  show ?thesis
    unfolding less_eq_cblinfun_def apply (rule exI[of _ \<open>sqrt \<delta> *\<^sub>R Y\<close>])
    by (auto simp: simp flip: scaleC_diff_left scaleR_scaleC)
qed
...
proof -
  ...
  hence "one_dim_iso ((vector_to_cblinfun \<psi>* o\<^sub>C\<^sub>L vector_to_cblinfun \<phi>) *\<^sub>V \<gamma>)
    ...
    for \<gamma> :: "'c::one_dim"
    by (simp add: of_complex_def scaleC_cblinfun.rep_eq)
  ...
  thus ?thesis
    using  cblinfun_ext[where A = "vector_to_cblinfun \<psi>* o\<^sub>C\<^sub>L vector_to_cblinfun \<phi>"
        and B = "\<langle>\<psi>, \<phi>\<rangle> *\<^sub>C id_cblinfun"]
    by auto
qed
...
lemma cblinfun_operator_basis_zero_uniq:
  ...
  by (metis UNIV_I a1 a2 a4 cblinfun.zero_left cblinfun_ext)
...
proof -
  ...
  have "B *\<^sub>S top = ccspan {x}"
    by (simp add: B \<phi>_def assms cblinfun_compose_image isometry_vector_to_cblinfun range_adjoint_isometry)
...
lemma one_dim_positive_op: \<open>positive_op A \<longleftrightarrow> one_dim_iso A \<ge> (0::complex)\<close>
proof -
  have \<open>positive_op A \<longleftrightarrow> (\<exists>B::'a \<Rightarrow>\<^sub>C\<^sub>L 'a. A = B* o\<^sub>C\<^sub>L B)\<close>
    unfolding positive_op_def by auto
  also have \<open>\<dots> \<longleftrightarrow> (\<exists>c::complex. A = (one_dim_iso c :: 'a\<Rightarrow>\<^sub>C\<^sub>L'a)* o\<^sub>C\<^sub>L (one_dim_iso c))\<close>
    by (smt (z3) Bounded_Operators.one_dim_iso_adjoint one_dim_iso_cblinfun_apply one_dim_iso_idem one_dim_iso_inj)
  also have \<open>\<dots> \<longleftrightarrow> (\<exists>c::complex. one_dim_iso A = cnj c * c)\<close>
    by (smt (z3) one_dim_iso_adjoint_complex one_dim_iso_cblinfun_apply_complex one_dim_iso_inj one_dim_iso_is_of_complex one_dim_iso_of_complex ordered_field_class.sign_simps(33))
  also have \<open>\<dots> \<longleftrightarrow> one_dim_iso A \<ge> (0::complex)\<close>
    apply (rule iffI)
     apply force
    apply (rule exI[of _ \<open>of_real (sqrt (cmod (one_dim_iso A :: complex)))\<close>])
    by (auto simp del: less_eq_complex_def simp flip: of_real_mult simp add: complex_of_real_cmod)
  finally show ?thesis
    by -
qed
